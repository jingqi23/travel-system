...
<script type="text/javascript">
			...
			jQuery.ajax({
			type:"post",//请求类型
			url:"${pageContext.request.contextPath }/tuan/query",//请求的url
			data:{queryFoodName:jQuery("#querygrogshopName").val(),pageIndex:1},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				options = " <h3>组团</h3><form class=\"form-horizontal\" ><div class=\"form-group\"><label for=\"inputEmail3\" class=\"col-sm-2 control-label\">名称</label><div class=\"col-sm-7\"><input type=\"text\" class=\"form-control\" id=\"querygrogshopName\" placeholder=\"请输入组团名称\"></div><div class=\" col-sm-2\"><button type=\"button\" class=\"btn btn-default\" onclick=\"sousuo(1)\">搜索</button></div></div></form>";
				if(data.tuanList.length>0){
				for(var i = 0; i < data.tuanList.length; i++){
						options += "<div class=\"destintn-grids\"><div class=\"col-md-4 desti-grid\"><a class='example-image-link' href='${pageContext.request.contextPath }/tuan/view?id="+data.tuanList[i].id+"\' ><img class=\"example-image\" src=\"images/"+data.tuanList[i].pic+"\"/><div class=\"desti-info\"><h5>"+data.tuanList[i].name+"</h5></div></a></div>";
				}
				}else{
					options +="空空如也！";
				}
			...
			function sousuo(pageIndex){
				...
				success:function(data){//data：返回数据（json对象）
					...
					if(data.tuanList.length>0){
					for(var i = 0; i < data.tuanList.length; i++){
							options += "<div class=\"destintn-grids\"><div class=\"col-md-4 desti-grid\"><a class='example-image-link' href='${pageContext.request.contextPath }/tuan/view?id="+data.tuanList[i].id+"\' ><img class=\"example-image\" src=\"images/"+data.tuanList[i].img1+"\"/><div class=\"desti-info\"><h5>"+data.tuanList[i].name+"</h5></div></a></div>";
					}
					}else{
						options +="空空如也！";
					}
