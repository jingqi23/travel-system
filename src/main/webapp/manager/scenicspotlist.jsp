                    ...
                    <div class="panel-body">
                        <div class="table-responsive">
                            <div id="dataTables-example_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                <form action="${pageContext.request.contextPath }/scenicspot/query1" method="post">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div id="dataTables-example_filter" class="dataTables_filter">
                                            <label>名称:<input type="search" class="form-control input-sm-8"
                                                             aria-controls="dataTables-example" name="queryScenicspotName" value="${queryScenicspotName }">
                                ...
                                <table class="table table-striped table-bordered table-hover dataTable no-footer"
                                    ...
                                    <tr role="row">
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1"
                                            ...
                                            style="width: 126px;">价格
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1"
                                            colspan="1" aria-label="Engine version: activate to sort column ascending"
                                            style="width: 126px;">点击
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1"
                                            colspan="1" aria-label="Engine version: activate to sort column ascending"
                                            style="width: 126px;">余票
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1"
                                    ...
                                    <tbody>
                                    <c:forEach var="scenicspot" items="${scenicspotList }" varStatus="status">
                                    <tr class="gradeA odd">
                                        <td class="sorting_1">${status.count }</td>
                                        <td class=" ">${scenicspot.name }</td>
                                        <td class=" ">${scenicspot.address }</td>
                                        <td class="center ">${scenicspot.price }元</td>
                                        <td class="center ">${scenicspot.dianji }次</td>
                                        <td class="center ">${scenicspot.tnum }</td>
                                        <td class="center ">
                                            <button type="button" class="btn btn-info" onclick="updatebyid(${scenicspot.id})" data-toggle="modal" data-target="#updateModal"><span
                                                    class="glyphicon glyphicon-edit"></span> 修改
                                            </button>
                                            <button type="button" onclick="doDel(${scenicspot.id})" class="btn btn-danger"><span
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">景点增加</h4>
            </div>
            <form role="form" id="addform" class="form-horizontal" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath }/scenicspot/add">
            	<div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                         <input class="form-control" id="addprice" name="price"/>
                        </div>
                        <span class="col-sm-1" style="color: red;">*</span>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">门票数:</label>
                        <div class="col-sm-8">
                         <input class="form-control" id="addtnum" name="tnum" type="number"/>
                        </div>
                        <span class="col-sm-1" style="color: red;">*</span>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">路线推荐:</label>
                        <div class="col-sm-8">
                       	<textarea class="form-control" rows="3" id="addsrr" name="srr"></textarea>
                        </div>
                    </div>
...
function add(){
	if($("#addname").val()=='' || $("#addname").val()==""){
	 ...
	 if($("#addprice").val()=='' || $("#addprice").val()==""){
		 alert("门票价格不能为空！");
			return false;
	 }
	 $("#addprice").val(priceReg($("#addprice").val()));
	 if($("#addtnum").val()==''){
		 alert("门票数量不能为空！");
			return false;
	 }
	 ...
	 if($("#addcoordinate").val()=='' || $("#addcoordinate").val()==""){
		 alert("坐标不能为空！");
			return false;
	 }
...
<div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="updateModalLabel">景点修改</h4>
            </div>
            <form role="form" id="updateform" class="form-horizontal" method="post" action="${pageContext.request.contextPath }/scenicspot/modifyexe">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">门票价格:</label>
                        <div class="col-sm-8">
                         <input class="form-control" id="updateprice" name="price"/>
                        </div>
                        <span class="col-sm-1" style="color: red;">*</span>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">门票数:</label>
                        <div class="col-sm-8">
                         <input class="form-control" id="updatetnum" name="tnum" type="number"/>
                        </div>
                        <span class="col-sm-1" style="color: red;">*</span>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">路线推荐:</label>
                        <div class="col-sm-8">
                       	<textarea class="form-control" rows="3" id="updatesrr" name="srr"></textarea>
                        </div>
                    </div>
                    <input type="hidden"  id="updateimg1" name="img1"/>
                    <input type="hidden"  id="updateimg2" name="img2"/>
                    <input type="hidden"  id="updateimg3" name="img3"/>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
...
function update(){
	if($("#updatename").val()=='' || $("#updatename").val()==""){
	 ...
	 if($("#updateprice").val()=='' || $("#updateprice").val()==""){
		 alert("门票价格不能为空！");
			return false;
	 }
	 $("#updateprice").val(priceReg($("#updateprice").val()));
	 if($("#updatetnum").val()==''){
		 alert("门票数量不能为空！");
			return false;
	 }
	 ...
	 if($("#updatecoordinate").val()=='' || $("#updatecoordinate").val()==""){
		 alert("坐标不能为空！");
			return false;
	 }
...
$.ajax({
	type:"post",//请求类型
	url:"${pageContext.request.contextPath }/scenicspot/modify",//请求的url
	data:{id:id},//请求参数
	...
	success:function(data){//data：返回数据（json对象）
		...
		$("#updatesrr").val(data.srr);
		$("#updatecoordinate").val(data.coordinate);
		$("#updatetnum").val(data.tnum);
		$("#updateimg1").val(data.img1);
		$("#updateimg2").val(data.img2);
		$("#updateimg3").val(data.img3);
	}});
}
function doDel(id){
	 if(confirm("是否删除?")){
	 	location.href="${pageContext.request.contextPath }/scenicspot/del/"+id;
	 }
}
</script>
